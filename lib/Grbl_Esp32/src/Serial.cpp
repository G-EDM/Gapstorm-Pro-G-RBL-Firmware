...
portMUX_TYPE input_mutex = portMUX_INITIALIZER_UNLOCKED;
...
void GRBL_SERIAL::client_reset_read_buffer() {
    inputBuffer.begin();
}
...
IRAM_ATTR int GRBL_SERIAL::client_read() {
    acquire_lock_for( ATOMIC_LOCK_INPUT );
    int c = inputBuffer.read();
    if( is_new_line( c ) ){
        while( is_new_line( inputBuffer.peek() ) ){
            inputBuffer.read(); // remove all following new lines
        }
    }
