...
GRBL_GCODE::GRBL_GCODE(){}


void GRBL_GCODE::gc_init() {
    memset(&gc_state, 0, sizeof(parser_state_t));
    gc_state.modal.coord_select = CoordIndex::G54; // Load default G54 coordinate system.
    coords[gc_state.modal.coord_select]->get(gc_state.coord_system);
}

void GRBL_GCODE::sync_xyz_block(){
    gc_sync_position();
    memcpy(gc_block.values.xyz, gc_state.position, sizeof(gc_state.position));
}

IRAM_ATTR void GRBL_GCODE::gc_sync_position() {
    system_convert_array_steps_to_mpos(gc_state.position, sys_position);
}

// Edit GCode line in-place, removing whitespace and comments and converting to uppercase
void GRBL_GCODE::collapseGCode(char* line) {
    char* parenPtr = NULL;
    char* outPtr = line;
    char  c;
...
Error GRBL_GCODE::gc_execute_line(char* line) {
    ...
    sync_xyz_block(); // hacky again. To make creating gcode easier z axis commands are allowed even if no z axis if available
    ...
    auto     n_axis          = N_AXIS;
    float    coord_data[N_AXIS];  // Used by WCO-related commands
           ...
           NOTE: Modal group numbers are defined in Table 4 of NIST RS274-NGC v3, pg.20 */
            ...
            case 'M':
                ...
                switch (int_value) {
                    ...
                    case 30:
                        ...
                        break;
                    case 3:
                        // used to raise the Z axis if the toolpath need to change
                        //planner.z_axis_up(); 
                        break;
                    case 4:
                        //planner.z_axis_down();
            ...
            default:
               ...
               words (I,J,K,L,P,R) have multiple connotations and/or depend on the issued commands. */
                ...
                switch (letter) {
                    case 'A':
                        if( axes_active.a.enabled ){
                            axis_word_bit               = GCodeWord::A;
                            gc_block.values.xyz[axes_active.a.index] = value;
                            axis_words |= bit(axes_active.a.index);
                        } else {
                            FAIL(Error::GcodeUnsupportedCommand);
                        }
                        break;
                    case 'B':
                        if( axes_active.b.enabled ){
                            axis_word_bit               = GCodeWord::B;
                            gc_block.values.xyz[axes_active.b.index] = value;
                            axis_words |= bit(axes_active.b.index);
                        } else {
                            FAIL(Error::GcodeUnsupportedCommand);
                        }
                        break;
                    case 'C':
                        if( axes_active.c.enabled ){
                            axis_word_bit               = GCodeWord::C;
                            gc_block.values.xyz[axes_active.c.index] = value;
                            axis_words |= bit(axes_active.c.index);
                        } else {
                            FAIL(Error::GcodeUnsupportedCommand);
                        }
                        break;
                    ...
                    case 'I':
                        ...
                        gc_block.values.ijk[0] = value;
                        ijk_words |= bit(0);
                        break;
                    case 'J':
                        axis_word_bit               = GCodeWord::J;
                        gc_block.values.ijk[1] = value;
                        ijk_words |= bit(1);
                        break;
                    case 'K':
                        axis_word_bit               = GCodeWord::K;
                        gc_block.values.ijk[2] = value;
                        ijk_words |= bit(2);
                        break;
                    case 'L':
                    ...
                    case 'T':
                        ...
                        break;
                    case 'X':
                        if( axes_active.x.enabled ){
                            axis_word_bit               = GCodeWord::X;
                            gc_block.values.xyz[axes_active.x.index] = value;
                            axis_words |= bit(axes_active.x.index);
                        } else {
                            FAIL(Error::GcodeUnsupportedCommand);
                        }
                        break;
                    case 'Y':
                        if( axes_active.y.enabled ){
                            axis_word_bit               = GCodeWord::Y;
                            gc_block.values.xyz[axes_active.y.index] = value;
                            axis_words |= bit(axes_active.y.index);
                        } else {
                            FAIL(Error::GcodeUnsupportedCommand);
                        }
                        break;
                    case 'Z':
                        if( axes_active.z.enabled ){
                            axis_word_bit = GCodeWord::Z;
                            gc_block.values.xyz[axes_active.z.index] = value;
                            axis_words |= bit(axes_active.z.index);
                        } else {
                            FAIL(Error::GcodeUnsupportedCommand);
                        }
                        break;
                    case 'U':
                        if( axes_active.u.enabled ){
                            axis_word_bit               = GCodeWord::U;
                            gc_block.values.xyz[axes_active.u.index] = value;
                            axis_words |= bit(axes_active.u.index);
                        } else {
                            FAIL(Error::GcodeUnsupportedCommand);
                        }
                        break;
                    case 'V':
                        if( axes_active.v.enabled ){
                            axis_word_bit               = GCodeWord::V;
                            gc_block.values.xyz[axes_active.v.index] = value;
                            axis_words |= bit(axes_active.v.index);
                        } else {
                            FAIL(Error::GcodeUnsupportedCommand);
                        }
                        break;
    ...
    */
    ...
    // Check for valid line number N value. removed..
    ...
    // feedrate section could be removed... #Todo
    if (gc_parser_flags & GCParserJogMotion) {
        if (bit_isfalse(value_words, bit(GCodeWord::F))) {
            gc_block.values.f = 50.0;
        }
        ...
        if (gc_block.modal.feed_rate == FeedRate::InverseTime) {  // = G93
            ...
            if (axis_command == AxisCommand::MotionMode) {
                if ((gc_block.modal.motion != Motion::None) || (gc_block.modal.motion != Motion::Seek)) {
                    if (bit_isfalse(value_words, bit(GCodeWord::F))) {
                        // to prevent error 22. Feedrate is not needed in EDM and we just set one
                        gc_block.values.f = 50.0;
                    }
                }
            }
    ...
    // spindle section could be removed... #todo
    ...
    switch (gc_block.modal.plane_select) {
        case Plane::XY:
            axis_0      = axes_active.x.index;
            axis_1      = axes_active.y.index;
            axis_linear = axes_active.z.index;
            break;
        case Plane::ZX:
            axis_0      = axes_active.z.index;
            axis_1      = axes_active.x.index;
            axis_linear = axes_active.y.index;
            break;
        default:  // case Plane::YZ:
            axis_0      = axes_active.y.index;
            axis_1      = axes_active.z.index;
            axis_linear = axes_active.x.index;
    }
    ...
    if (axis_command != AxisCommand::None) {
        bit_false(value_words,
                  (bit(GCodeWord::X) | bit(GCodeWord::Y) | bit(GCodeWord::Z) | 
                   bit(GCodeWord::U) | bit(GCodeWord::V) | bit(GCodeWord::A) | 
                   bit(GCodeWord::B) | bit(GCodeWord::C)));  // Remove axis words.
    }
    if (value_words) {
    ...
    */
    ...
    // Intercept jog commands and complete error checking for valid jog commands and execute. NOTE: G-code parser state is not updated, except the position to ensure sequential jog
    // targets are computed correctly. The final parser position after a jog is updated in protocol_exec_rt_system() when jogging completes or is canceled.
    if (gc_parser_flags & GCParserJogMotion) {
        ...
        Error status            = motion.jog( pl_data, &gc_block );
        if ( status == Error::Ok ) {
    ...
    if (gc_block.modal.tool_change == ToolChange::Enable) {
        //grbl.user_tool_change(gc_state.tool);
    }
    // [7. Spindle control ]:
    ...
    // state can have both mist (M7) and flood (M8) on at once, by issuing M7 and M8 in separate blocks.  There is no GCode way to turn them off separately, but
    // you can turn them off simultaneously with M9.  You can turn them off separately with real-time overrides, but that is out of the scope of GCode.
    ...
    // [10. Dwell ]:
    if (gc_block.non_modal_command == NonModal::Dwell) {
        motion.dwell(int32_t(gc_block.values.p * 1000.0f));
    }
    // [11. Set active plane ]:
    ...
    // of execution. The error-checking step would simply load the offset value into the correct axis of the block XYZ value array.
    ...
    switch (gc_block.non_modal_command) {
        ...
        case NonModal::GoHome1:
            ...
            if (axis_command != AxisCommand::None) {
                glimits.limits_check_soft(gc_block.values.xyz);
                motion.line(gc_block.values.xyz, pl_data);
                //cartesian_to_motors(gc_block.values.xyz, pl_data, gc_state.position);
            }
            glimits.limits_check_soft(coord_data);
            motion.line(coord_data, pl_data);
    ...
    if (gc_state.modal.motion != Motion::None) {
        if (axis_command == AxisCommand::MotionMode) {
            ...
            if (gc_state.modal.motion == Motion::Linear) {
                ...
                motion.arc(gc_block.values.xyz,
                       pl_data,
                       gc_state.position,
                       gc_block.values.ijk,
                       gc_block.values.r,
                ...
                gc_update_pos = motion.probe_cycle(gc_block.values.xyz, pl_data, gc_parser_flags);
                set_machine_state( STATE_BUSY );
            }
            // As far as the parser is concerned, the position is now == target. In reality the
            // motion control system might still be processing the action and the real tool position
            // in any intermediate location. (Code is now blocking so this should not be a thing anymore..)
