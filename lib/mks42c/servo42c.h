...
#include "mks42cconfig.h"


class SERVO42C {

    protected:
        uint8_t  motor_speed; // 0-127 (7bit)
        uint16_t microsteps;
        uint16_t max_torque;
        uint16_t max_current;
        bool     motor_type; // true = 1.8°; false= 0.9° step angle
    ...
    private:
        ...
        static int MKS_SLAVE_ADDRESS_CURRENT;

        static HardwareSerial *_serial; // if needed it will be UART2 with custom pins for RX/TX
        ...
        bool    receive( uint8_t* response, uint8_t receive_length = MKS_DEFAULT_RECEIVE_LENGTH, uint8_t command_code = 0x00 );
        
    public:
        SERVO42C();
        virtual ~SERVO42C() = default;

        bool    begin( int uart_address = 0 );
        bool    set_mks_speed( uint8_t _speed );
        ...
        bool    set_slave_address( uint8_t address_num = 0, bool write = true );
        ...
        uint16_t get_steps_per_mm( void );

        uint16_t get_setting_max_torque( void );
        uint16_t get_setting_max_current( void );

        bool is_available( void );
        void set_is_available( bool state );

        float get_rpm_for_speed( uint8_t _speed );
        uint16_t get_steps_per_rev( void );
