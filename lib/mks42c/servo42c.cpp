...
HardwareSerial *SERVO42C::_serial        = nullptr;
bool SERVO42C::serial_initialized        = false;
int  SERVO42C::MKS_SLAVE_ADDRESS_CURRENT = -1;
static int MAX_ADDRESSES = 9;

bool SERVO42C::begin( int uart_address ){

    if( MKS_SLAVE_ADDRESS_CURRENT == MAX_ADDRESSES ){
        debuglog("*  No addresses left");
    } else {
        ++MKS_SLAVE_ADDRESS_CURRENT; // increments with each new motor
    }

    if( uart_address < MKS_SLAVE_ADDRESS_CURRENT || uart_address > MAX_ADDRESSES ){
        debuglog("*  UART address not available");
        uart_address = MKS_SLAVE_ADDRESS_CURRENT;
    }

    if( !serial_initialized ){
        debuglog("UART2 init", DEBUG_LOG_DELAY );
        // initialize hardware serial 2
        // this is a static variable and will only get called with the first creation of a class instance
        _serial = new HardwareSerial(2);
        _serial->setRxBufferSize(1024);
        _serial->begin( MKS42C_BAUDRATE, SERIAL_8N1, RX_PIN, TX_PIN );
        serial_initialized = true;
    }

    set_slave_address( uart_address, false );
    set_mks_speed( MKS42C_MOTOR_SPEED_DEFAULT );

    microsteps       = MKS42C_MICROSTEPS_DEFAULT;
    motor_speed      = MKS42C_MOTOR_SPEED_DEFAULT;
    max_torque       = MKS42C_MAXTORQUE_DEFAULT;
    max_current      = MKS42C_MAXCURRENT_DEFAULT;
    motor_type       = MKS42C_MOTOR_TYPE_DEFAULT == 1 ? true : false;
    interpolation_on = MKS42C_ENABLEMICROSTEPS_DEFAULT == 1 ? true : false;
    available        = true;
...
uint16_t SERVO42C::get_steps_per_rev( void ){
    return ( microsteps * ( motor_type ? 200 : 400 ) );
}


float SERVO42C::get_rpm_for_speed( uint8_t _speed ){
    return ( float( ( _speed * 30000 ) ) / float( get_steps_per_rev() ) );
}


bool SERVO42C::set_mks_speed( uint8_t _speed ){
    if( _speed > 127 ){ _speed = 127; }
    _speed &= 0x7F; // redundant
    motor_speed = _speed;
    return true;
}
...
//##################################################################
bool SERVO42C::set_slave_address( uint8_t value, bool write ){
    if( value > 9 ){ value = 9; }
...
bool SERVO42C::send( uint8_t *hex_block_set, size_t hex_block_size, uint8_t *response, uint8_t receive_length ){
    ...
    bool    success = false;
    do{
        if( retry > 1 ){ vTaskDelay(1); }
        _serial->flush();
        _serial->write( hex_block_set, hex_block_size ); // E0, A5, 00, 01, 0x86
        success = receive( response, receive_length, hex_block_set[1] );
...
//#########################################################################
bool SERVO42C::receive( uint8_t* response, uint8_t receive_length, uint8_t command_code ){
    ...
    while(1){
        ...
        /*if( command_code == CMD_SET_RUN_BY_STEPNUM ){
            // make sure motors stops on estop etc.
    ...
    while( _serial->available() > 0 ){ // flush RX buffer
        _serial->read();
    }
...
// Range: 0 - 255 (translates to 1-256)
//
// UART return:
// status 1 = Set success - status 0 = Set failed
//
...
bool SERVO42C::set_subdivision( uint8_t value ){
    ...
    if( status == 1 ){
        microsteps = value+1; // (1-256 translates to 0-255 to fit in a byte)
        return true;
    }
    return false;
}
...
// Function return:
// true = success, false = error
// I think this results in 256 microstepping 
//##################################################################
bool SERVO42C::set_subdivision_interpolation( uint8_t value ){
    ...
    if( status == 1 ){
        interpolation_on = value == 1 ? true : false;
        return true;
    }
    return false;
}
...
//.....
uint16_t SERVO42C::get_steps_per_mm(){
    /*int steps     = 0;
    int fullsteps = 200;
    int steps_per_rev;
    if( !motor_type){
        fullsteps = 400; // 0.9° stepper
    }
    if( interpolation_on ){
        steps_per_rev = fullsteps * 256;
    } else {
        steps_per_rev = fullsteps * microsteps;
    }*/
    return 0;
}
...
//##########################################
// MKS42C does not provide any features to
// read those settings. So in order to know
// the setting value it needs to get written
// first.
//##########################################
uint16_t SERVO42C::get_setting_max_torque(){
    return max_torque;
}
uint16_t SERVO42C::get_setting_max_current(){
    return max_current;
}


bool SERVO42C::is_available(){
    return available;
}
void SERVO42C::set_is_available( bool state ){
    available = state; // false = not available
}